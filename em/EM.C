...
int main(int argc, char **argv){
	...
	while(1){
		...
		for(unsigned int cls = 0; cls < classes.size(); ++cls){
			...
			if(iteration == niteration) {
				error[cls] = xx*xx + xy*xy + yy*yy;
				printf("class %d: mean=(%.3f,%.3f) xx=%.3f yy=%.3f xy=%.3f error:%.3f\n",
					cls, mean.x, mean.y, xx, yy, xy, error[cls]);
				distoration += error[cls];
			}
	...
	printf("Total distoration: %f", distoration);
	printf("\nplot");
/*
